...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.querySelector("#sendrecv1-connect")?.addEventListener("click", async () => {
    ...
    const stream = await getAudioStreamWithDevice(selectedAudioDeviceId, "sendrecv1");
  ...
  document.querySelector("#sendrecv2-connect")?.addEventListener("click", async () => {
    ...
    const selectedAudioDeviceId = audioInputSelect?.value;
    const stream = await getAudioStreamWithDevice(selectedAudioDeviceId, "sendrecv2");
...
async function getAudioStreamWithDevice(
  ...
  if (isIOSSafari && deviceId) {
    try {
      ...
      if (audioTrack) {
        ...
        // 画面に使用されたデバイスIDを表示
        const usedDeviceElement = document.querySelector<HTMLDivElement>(
          `#${clientId}-used-device`,
        );
    ...
    if (audioTrack) {
      ...
      // 画面に使用されたデバイスIDを表示
      const usedDeviceElement = document.querySelector<HTMLDivElement>(`#${clientId}-used-device`);
      if (usedDeviceElement) {
        usedDeviceElement.textContent = `使用中のデバイスID: ${audioTrack.getSettings().deviceId}`;
      }
    }
